...
!developmentChains.includes(network.name)
  ? describe.skip
  : describe("DGNM Buy Offer Logic Unit Tests", () => {
      let owner;
      let nftContract;
      let collectorsNftContract;
      let marketContract;
      let erc20Mock;
      ...
      async function deployNFTAndMarketContracts() {
        ...
        nftContract = await deployContract("DGNMCollection", [
          collectorsNftContract.target,
          mintFee,
        ]);
        ...
        marketContract = await deployContract("DGNMMarket", [
          nftContract.target,
        ]);
      }
      ...
      before(async () => {
        ...
        collectorsNftContract = await deployContract("DGNMCollectors", []);

        // register user1, user2, user3, randomUser
        const TEST_URI = "ipfs://test-nft-profile-uri";

        await collectorsNftContract.connect(user1).create(TEST_URI);
        await collectorsNftContract.connect(user2).create(TEST_URI);
        await collectorsNftContract.connect(user3).create(TEST_URI);
        await collectorsNftContract.connect(randomUser).create(TEST_URI);
      });
      ...
      describe("makeOffer()", () => {
        describe("ERC20 token payment offer", () => {
          ...
          it("should not allow user to make offre with unsupported payment token", async () => {
            ...
            await expect(
              ...
              marketContract,
              "DGNMMarket_UnsupportedToken"
            );
          });
          it("should not allow user to make offer on non DGNM token", async () => {
          ...
          it("should revert when expiration time is in the past", async () => {
            ...
            await expect(
              ...
              marketContract,
              "DGNMMarket_InvalidExpirationTime"
            );
          });
          it("should revert when offer price is not approved for market", async () => {
            await expect(
              ...
              marketContract,
              "DGNMMarket_OfferAmountNotApproved"
            );
          });
          it("should allow user to make offer on existing DGNM token", async () => {
            await approveERC20(
              randomUser,
              erc20Mock.target,
              price,
        ...
        describe("DTOKEN payment offer", () => {
          ...
          it("should revert when insufficient DTOKEN amount is sent", async () => {
            // mint new NFT to user1
            await mintNewNFT(nftContract, user1);
